...
from lib.models.mylayer.attn_window import Semantic_guided_CMWA
from lib.models.bat.utils import combine_tokens
...
class SG_CMWABlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.SG_CMWA_1 = Semantic_guided_CMWA()
        self.SG_CMWA_2 = Semantic_guided_CMWA()
    ...
    def forward(self, x, xi, global_index_template, global_index_templatei, global_index_search, global_index_searchi, mask=None, ce_template_mask=None, keep_ratio_search=None):
        ...
        lens_x = global_index_search.shape[1]
        lens_z = global_index_template.shape[1]
        lens_xi = global_index_searchi.shape[1]
        lens_zi = global_index_templatei.shape[1]
        rgb_template = x[:, :lens_z]
        rgb_search = x[:, lens_z:]
        ir_template = xi[:, :lens_zi]
        ir_search = xi[:, lens_zi:]
        xori = x
        
        x_attn, attn = self.attn(self.norm1(x), mask, True)   
        x = x + self.drop_path(x_attn) + self.drop_path(self.adap_t(combine_tokens(rgb_template,self.SG_CMWA_1(self.norm1(rgb_search),self.norm1(ir_search),self.norm1(rgb_template),self.norm1(ir_template)))))
        ...
        x = x + self.drop_path(self.mlp(self.norm2(x))) + self.drop_path(self.adap2_t(combine_tokens(rgb_template,self.SG_CMWA_2(self.norm2(rgb_search),self.norm2(ir_search),self.norm2(rgb_template),self.norm2(ir_template)))))
...
if __name__ == '__main__':
    B=2
    z = torch.randn(B, 64, 768)
    x = torch.randn(B, 256, 768)
    zi = torch.randn(B, 64, 768)
    xi = torch.randn(B, 256, 768)
    ceab = SG_CMWABlock(768, 8, 0.1)
    pos_embed_z = nn.Parameter(torch.zeros(1, 64, 768))
    pos_embed_x = nn.Parameter(torch.zeros(1, 256, 768))
    lens_z = pos_embed_z.shape[1]
    lens_x = pos_embed_x.shape[1]
    ...
    xi = combine_tokens(zi,xi)
    x, global_index_t, global_index_s, removed_index_s, attn, xi, global_index_ti, global_index_si, removed_index_si, i_attn = ceab(x, xi, global_index_t, global_index_ti, global_index_s, global_index_si, keep_ratio_search=0.5)
    print(x.shape)
    print(xi.shape)