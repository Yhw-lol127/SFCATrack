...
from ..layers.dualstream_attn_blocks import DSBlock 

from lib.models.layers.dcn_layer import MMAF
...
class VisionTransformerCE(VisionTransformer):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.mmaf_block = []
        for i in range(depth):
            self.mmaf_block.append(
                MMAF()
            )
        self.mmaf_blocks = nn.Sequential(*self.mmaf_block)
...
###################################################################===========
        ...
        for i, blk in enumerate(self.blocks):
            ...
            x_fusion = self.mmaf_blocks[i](x, xi)
            x = x + x_fusion
            xi = xi + x_fusion
