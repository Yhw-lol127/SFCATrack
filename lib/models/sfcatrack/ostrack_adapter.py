...
from lib.models.sfcatrack.vit_ce_adapter import vit_base_patch16_224_ce_adapter
from lib.utils.box_ops import box_xyxy_to_cxcywh
from lib.models.layers.moe_block import BiasNetwork


class SFCATrack(nn.Module):
    ...
    def forward(self, template: torch.Tensor,
        ...
        if not self.training:
            if stage == "phase1":
                ...
                template_ir = template[:, 3:, :, :]
                search_rgb = search[:, :3, :, :]
                search_ir = search[:, 3:, :, :]
                search_rgb_prior = search_prior[:, :3, :, :]
                search_ir_prior = search_prior[:, 3:, :, :]
                bias = self.offset_pred(search_prior, stage, expert_index)
                return bias
            else:
            ...
            if stage == "att_moe_phase1" or stage == "att_moe_phase2" or stage == "att_moe_phase3":
                # print("Only train Bias Net...")
                bias = self.offset_pred(search_prior, stage, expert_index)
                return bias
            
            
            else:
                # print("Only train Backbone...")
                x, aux_dict = self.backbone(z=template, x=search,
                                            ce_template_mask=ce_template_mask,
                                            ce_keep_rate=ce_keep_rate,
                                            return_last_attn=return_last_attn, )
...
def build_sfcatrack(cfg, training=True):
    ...
    if 'OSTrack' in cfg.MODEL.PRETRAIN_FILE and training:
        ...
        checkpoint_keys = checkpoint["net"].keys()
        correct_keys = model_keys&checkpoint_keys
    elif pretrained and training:
