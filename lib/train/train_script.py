...
# network related
from lib.models.sfcatrack import build_ostrack
from lib.models.sfcatrack import build_sfcatrack
# forward propagation related
from lib.train.actors import SFCAActor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "sfcatrack":
        net = build_sfcatrack(cfg)
        #print(net)
    else:
    ...
    if settings.script_name == "sfcatrack":
        ...
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT, 'focal': 1., 'cls': 1.0, 'bias': 1.0}
        actor = SFCAActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
