...
def get_x_frame(color_path, depth_path, dtype='rgbcolormap', depth_clip=False):
    ...
    if dtype == 'color':
        ...
        #dp = cv2.cvtColor(dp, cv2.COLOR_BGR2RGB)
        dp =cv2.resize(dp, (rgb.shape[1], rgb.shape[0]))
        img = cv2.merge((rgb, dp))
    elif dtype == 'rgbrgb_notalign_1':
        if dp is not None:
                dp_resized = cv2.resize(dp, (rgb.shape[1], rgb.shape[0]))
                img_rgb_size = cv2.merge((rgb, dp_resized))
                rgb_resized = cv2.resize(rgb, (dp.shape[1], dp.shape[0]))
                img_tir_size = cv2.merge((rgb_resized, dp))
